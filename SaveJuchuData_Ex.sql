IF EXISTS(SELECT * FROM sysobjects WHERE name='SaveJuchuData_Ex')
   DROP PROCEDURE dbo.SaveJuchuData_Ex
GO

CREATE PROCEDURE SaveJuchuData_Ex
...
BEGIN
	...
	DECLARE @FLD_NM_JUCHU_KBN					NVARCHAR(20)	= '(受注区分)'
	DECLARE @FLD_NM_JUCHU_DATE					NVARCHAR(20)	= '(受注日)'
	DECLARE @FLD_NM_SHUKKO_KBN					NVARCHAR(20)	= '(出庫区分)'
	DECLARE @FLD_NM_TOKUISAKI_CD				NVARCHAR(20)	= '(得意先コード)'
	DECLARE @FLD_NM_SHUKKASAKI_CD				NVARCHAR(20)	= '(出荷先コード)'
	DECLARE @FLD_NM_SEIKYUSAKI_CD				NVARCHAR(20)	= '(請求先コード)'
	DECLARE @FLD_NM_SOKO_CD						NVARCHAR(20)	= '(倉庫コード)'
	DECLARE @FLD_NM_AZUKE_AZUKARI_IDO_SOKO_CD	NVARCHAR(20)	= '(預け／預かり／移動倉庫コード)'
	DECLARE @FLD_NM_HDR_SHUKKA_YOTEI_DATE		NVARCHAR(20)	= '(ヘッダ出荷予定日)'
	DECLARE @FLD_NM_DTL_SHUKKA_YOTEI_DATE		NVARCHAR(20)	= '(明細出荷予定日)'
	DECLARE @FLD_NM_SHITEI_NOHIN_DATE			NVARCHAR(20)	= '(指定納品日)'
	DECLARE @FLD_NM_SHITEI_NOHIN_JIKOKU			NVARCHAR(20)	= '(指定納品時刻)'
	DECLARE @FLD_NM_HAISOU_KBN					NVARCHAR(20)	= '(配送区分)'
	DECLARE @FLD_NM_KESSAI_HOHO					NVARCHAR(20)	= '(決済方法)'
	DECLARE @FLD_NM_GEDAIMACHI_FLG				NVARCHAR(20)	= '(下代待ちフラグ)'
	DECLARE @FLD_NM_SAMPLE_SHUKKA_FLG			NVARCHAR(20)	= '(サンプル出荷フラグ)'
	DECLARE @FLD_NM_HIKIATE_CHOSEI_FUYO_FLG		NVARCHAR(20)	= '(引当調整不用フラグ)'
	DECLARE @FLD_NM_SEIKYU_KIJUN_DATE			NVARCHAR(20)	= '(請求基準日)'
	DECLARE @FLD_NM_TOKUISAKI_DENPYO_NO			NVARCHAR(20)	= '(得意先伝票NO)'
	DECLARE @FLD_NM_TOKUISAKI_HACCHU_NO			NVARCHAR(20)	= '(得意先発注NO)'
	DECLARE @FLD_NM_BUNRUI_CD					NVARCHAR(20)	= '(分類コード)'
	DECLARE @FLD_NM_URIBA_NM					NVARCHAR(20)	= '(売場名)'
	DECLARE @FLD_NM_TANTOSHA_NM					NVARCHAR(20)	= '(担当者名)'
	DECLARE @FLD_NM_NAISEN_NO					NVARCHAR(20)	= '(内線番号)'
	DECLARE @FLD_NM_TANTOSHA_CD					NVARCHAR(20)	= '(担当者コード)'
	DECLARE @FLD_NM_TANTO_BUSHO_CD				NVARCHAR(20)	= '(担当部署コード)'
	DECLARE @FLD_NM_NOHIN_KBN					NVARCHAR(20)	= '(納品区分)'
	DECLARE @FLD_NM_RYUTU_KAKOU_KBN				NVARCHAR(20)	= '(流通加工区分)'
	DECLARE @FLD_NM_BIKO						NVARCHAR(20)	= '(備考)'
	DECLARE @FLD_NM_JUCHU_COMMENT				NVARCHAR(20)	= '(受注コメント)'
	DECLARE @FLD_NM_SOKO_COMMENT				NVARCHAR(20)	= '(倉庫コメント)'
	DECLARE @FLD_NM_PROJECT_CD					NVARCHAR(20)	= '(プロジェクトコード)'
	DECLARE @FLD_NM_HANBAI_AREA_CD				NVARCHAR(20)	= '(販売エリアコード)'
	DECLARE @FLD_NM_YOYAKU_KAIHO_KIGEN			NVARCHAR(20)	= '(予約解放期限)'
	DECLARE @FLD_NM_HINMOKU						NVARCHAR(20)	= '(品目)'
	DECLARE @FLD_NM_LOT_NUM						NVARCHAR(20)	= '(ロット数)'
	DECLARE @FLD_NM_BARA_NUM					NVARCHAR(20)	= '(バラ数)'
	DECLARE @FLD_NM_SUITEI_KAKUTEI_KBN			NVARCHAR(20)	= '(推定確定区分)'
	DECLARE @FLD_NM_KAKERITU					NVARCHAR(20)	= '(掛率)'
	DECLARE @FLD_NM_JUCYU_TANKA					NVARCHAR(20)	= '(受注単価)'
	DECLARE @FLD_NM_HIKIATE_STATE				NVARCHAR(20)	= '(引当状態)'
	DECLARE @FLD_NM_TEKIYO						NVARCHAR(20)	= '(摘要)'
	DECLARE @FLD_NM_YOYAKU_DENPYO_NO			NVARCHAR(20)	= '(予約伝票NO)'
	DECLARE @FLD_NM_YOYAKU_DENPYO_ENO			NVARCHAR(20)	= '(予約伝票枝番)'
	DECLARE @FLD_NM_KAZEI_KBN					NVARCHAR(20)	= '(課税区分)'
	DECLARE @FLD_NM_TORIHIKISAKI_TUKA_TANKA		NVARCHAR(20)	= '(取引先通貨単価)'
	DECLARE @FLD_NM_RATE						NVARCHAR(20)	= '(レート)'
    ...
    **********************************************/
	--未設定は受注日以上の翌営業日
	UPDATE #tmp
	SET SHUKKA_YOTEI_DATE = dbo.CO_FUNC_GET_NEXT_EIGYO_DATE_EX(@KAISHA_CD, JUCHU_DATE)
	WHERE ERROR_MSG IS NULL
	AND SHUKKA_YOTEI_DATE IS NULL
	AND ISDATE(JUCHU_DATE) = 1
	/**********************************************
    *ヘッダ出荷予定日設定END
    **********************************************/
	...
	SET #tmp.ERROR_MSG = 
		CASE --在庫管理品の場合に、倉庫未設定
			 ...
			 WHEN JUCHU_KBN ='01' AND #tmp.LOT_NUM IS NULL AND #tmp.BARA_NUM IS NULL AND HIN.STD_IRISU IS NULL
					THEN @ERR_MSG_MANDATORY_FIELD + @FLD_NM_LOT_NUM + @FLD_NM_BARA_NUM
		END
	FROM #tmp
	INNER JOIN BC_MST_HINMOKU_KANRI HINKAN
		ON HINKAN.HINMOKU_SEQ = #tmp.HINMOKU_SEQ
    ...
    **********************************************/
	UPDATE #tmp
	SET ERROR_MSG = @ERR_MSG_DIFFERENT_TUKA + @FLD_NM_TOKUISAKI_CD + @FLD_NM_SEIKYUSAKI_CD
	FROM #tmp tmp
    ...
    **********************************************/
	...
	SET ERROR_MSG = 
			CASE WHEN SHUKKO_KBN <> '01'
					THEN @ERR_MSG_CANNOT_USE_CODE + @FLD_JUCHU_KBN_YOYAKU + @FLD_NM_SHUKKO_KBN
				 WHEN SUITEI_KAKUTEI_KBN <> '01'
					THEN @ERR_MSG_CANNOT_USE_CODE + @FLD_JUCHU_KBN_YOYAKU + @FLD_NM_SUITEI_KAKUTEI_KBN
			END
	WHERE ERROR_MSG IS NULL
	AND JUCHU_KBN = '02'
	...
	SET ERROR_MSG = CASE WHEN SHUKKO_KBN <> '40'
							THEN @ERR_MSG_CANNOT_USE_CODE + @FLD_JUCHU_KBN_SOKO_IDO + @FLD_NM_SHUKKO_KBN
					END
	WHERE ERROR_MSG IS NULL
	AND JUCHU_KBN = '03'
    ...
    **********************************************/
    ...
    SET #tmp.ERROR_MSG = 
			CASE WHEN SOKO.SOKO_KBN <> '01'
					THEN @ERR_MSG_TUJOU_SHUKKO_SOKO + @FLD_NM_SOKO_CD + #tmp.DTL_SOKO_CD
			END
	FROM #tmp
	INNER JOIN BC_MST_SOKO SOKO
    ...
    **********************************************/
	...
	SET #tmp.ERROR_MSG = 
		CASE --指定納品時刻
			 ...
			 --担当者名
			 WHEN TANTOSHA_NM IS NOT NULL AND LEN(TANTOSHA_NM) > 20
			 ...
			 --摘要
			 WHEN TEKIYO IS NOT NULL AND LEN(TEKIYO) > 30
    ...
    **********************************************/
	...
	SET #tmp.ERROR_MSG = 
			CASE WHEN DATEDIFF(DD, #tmp.JUCHU_DATE, #tmp.SHUKKA_YOTEI_DATE) < 0
						THEN @ERR_MSG_INCORRECT_DATA + @FLD_NM_JUCHU_DATE + '＞' + @FLD_NM_HDR_SHUKKA_YOTEI_DATE
				 WHEN DATEDIFF(DD, #tmp.JUCHU_DATE, #tmp.DTL_SHUKKA_YOTEI_DATE) < 0
						THEN @ERR_MSG_INCORRECT_DATA + @FLD_NM_JUCHU_DATE + '＞' + @FLD_NM_DTL_SHUKKA_YOTEI_DATE
			 END
	WHERE #tmp.ERROR_MSG IS NULL
	/**********************************************
    *日付チェックEND
    ...
    **********************************************/
	...
	SET #tmp.ERROR_MSG = 
		CASE --ロット数
			 ...
			 --取引先通貨単価
			 WHEN (CONVERT(float, TORIHIKISAKI_TUKA_TANKA) < 0
					OR (9999999999.999<CONVERT(float,TORIHIKISAKI_TUKA_TANKA)))
    ...
    **********************************************/
	...
	UPDATE #tmp
    SET #tmp.ERROR_MSG = @ERR_MSG_CANNOT_USE_CODE + @FLD_NM_HIKIATE_STATE + @FLD_NM_SUITEI_KAKUTEI_KBN
	WHERE #tmp.ERROR_MSG IS NULL
	...
	*エラー無行に対してのみ引当処理を実施
	*引当は出荷予定日の昇順、受注区分(本受注：01＞予約：02＞移動：03)の昇順で実施
	*在庫管理外品は引当数を注文数とする
	*************************************************************************************/
	--出荷予定作成用TBL
	...
	ORDER BY DTL_SHUKKA_YOTEI_DATE
			,JUCHU_KBN
			,LINE_NO
	...
	BEGIN
		...
		SELECT @KAISHA_CD						--会社コード
			  ...
			  ,NULL								--商品ロット
			  ,CONVERT(DATE,DTL_SHITEI_NOHIN_DATE)			--入荷予定日
			  ,NULL								--入荷予定時区分
			  ,NULL								--検収予定日
			  ,NULL								--入数
			  ,NULL								--箱数
		...
		SELECT @KAISHA_CD				--会社コード
			  ...
			  ,NULL						--商品ロット
			  ,CASE 					--入荷予定日
			  		WHEN DTL_SHITEI_NOHIN_DATE IS NOT NULL
			  			THEN CONVERT(DATE, DTL_SHITEI_NOHIN_DATE)
			  			ELSE CONVERT(DATE, DTL_SHUKKA_YOTEI_DATE)
			  	END
			  ,NULL						--検収予定日
			  ,NULL						--入数
			  ,NULL						--箱数
			  ...
			  ,NULL						--入庫日
			  ,NULL						--検収日
			  ,CONVERT(DATE, DTL_SHITEI_NOHIN_DATE)	--資産計上開始日(アドオンで品目の開始日数はなくなる)
	    ...
	    **********************************************/
		...
		--登録伝票NO取得
		SELECT MIN(DENPYO_NO) AS MIN_DENPYO_NO
